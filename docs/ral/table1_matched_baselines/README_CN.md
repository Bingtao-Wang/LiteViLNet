...
RoadFormer 和 LiteViLNet 全部使用同一组 KITTI 训练/验证样本、同一 `384 x 1248` 输入尺寸、
...
perspective-view 结果直接排名。按此规则，USNet/SNE-RoadSeg 覆盖 CNN 融合路线，
...
- 四个官方源码目录的 remote、commit、语义 diff 和关键文件哈希都会由脚本复核；
  当前正式实验使用的 PLARD/RoadFormer 是 clean fresh clone，USNet/SNE-RoadSeg
...
RoadFormer 的依据是官方 KITTI config 及其 vendored OpenMMLab 模块。这些文件的 Git blob 与本次工作副本 SHA-256 均列在
...
RGB--ADI 网络、三路监督与 SGD；保留 RoadFormer 的 TwinConvNeXt-B、Hungarian
matching losses、AdamW 和 Transformer decoder。我们新增的部分仅为：

1. 让四种官方模型读取同一份无泄漏 231/58 manifest；
...
因此，补充材料中的四种 baseline 数值是“官方模型与官方训练 recipe + 本文统一
...
6. `train_matched_kitti_roadformer.py`
   - 直接注册官方 Python graph，不修改第三方源码；
   - 把官方 `mmcv_custom` 算子名映射到同 ABI 的 MMCV 1.7 CUDA 算子；
   - 使用官方 FP32 路径、TwinConvNeXt-B 初始化、loss、AdamW 和梯度裁剪。
7. `run_matched_kitti_baselines.sh`
   - 在两张 GPU 上各顺序运行两种方法的三个 seed；
...
13. `package_table1_matched_baselines.sh`
   - 检查十二份逐 seed JSON、五份 FPS JSON 与汇总证据是否完整；
...
RoadFormer 使用其官方代码兼容的独立旧版 OpenMMLab 环境：
...
训练和五方法统一 FPS-1 都明确使用 FP32，不用隐式精度差异制造速度优势。
...
最后在两张 GPU 上分别顺序训练四种方法的三个 seed，并自动汇总：
...
`train_matched_kitti_baseline.py` 或 `train_matched_kitti_roadformer.py`。不要把十二个训练进程的 wall-clock 用作 Table I
...
精度训练完成后，使用每种方法任一完成 seed 的 best checkpoint 测速。五种方法必须
...
五种方法可以在激活 `litevilnet_ral` 后用统一入口顺序测速；RoadFormer 会由脚本自动
切到其独立环境，并在全部完成后自动汇总：
...
汇总器会核对五个 JSON 的 GPU 名、精度、输入、timing scope、重复次数、参数量、
...
未四舍五入的指标、best epoch、官方源码哈希和十二个 best checkpoint SHA-256 位于
`results/summary.json` 与 `results/seeds/*.json`。LiteViLNet 主行由完整匿名附件中的
主训练队列生成，结果为 `97.23 ± 0.15%` MaxF、`14.04M` 参数；Table I 因此可以直接
核对为同 split/尺寸/预算/evaluator 的五方法结果。
...
只有在十二份逐 seed JSON、五份 FPS JSON 和汇总结果都存在时才会执行成功。它在临时目录中把绝对
路径替换为 `$USNET_SOURCE`、`$SNE_SOURCE`、`$PLARD_SOURCE`、`$ROADFORMER_SOURCE`、`$KITTI_ROOT` 等可移植占位符，且不会
改写本机原始 JSON；随后扫描 home/data 路径、邮箱、SSH remote，以及通过
`LITEVILNET_DOUBLE_BLIND_TOKENS` 提供的逗号分隔身份关键词，并把 tar
